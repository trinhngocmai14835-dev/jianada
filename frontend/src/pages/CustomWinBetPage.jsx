...
export default function CustomWinBetPage() {
  ...
  const refresh = async () => {
    ...
    setStatus(s.custom_winbet || 'stopped')
    const accountStatus = await api.getCustomWinBetAccountStatuses()
    setAccounts(accountStatus.accounts || [])
  }

  useEffect(() => {
    api.getCustomWinBetConfig().then((cfg) => form.setFieldsValue(cfgToForm(cfg)))
    refresh()
  ...
  const doStart = async () => {
    setLoading(true)
    const res = await api.startCustomWinBet()
  ...
  const handleStop = async () => {
    setLoading(true)
    const res = await api.stopCustomWinBet()
    message.info(res.message)
    setLoading(false)
    refresh()
  }
  ...
  const handleStopAccount = async (key) => {
    const res = await api.stopCustomWinBetAccount(key)
    message.info(res.message)
    refresh()
  }
      ...
      <Row gutter={24}>
        ...
        <Col xs={24} lg={10}>
          ...
          <Card title="实时日志" style={{ borderRadius: 8 }}>
            <LogViewer taskId="custom_winbet" running={isRunning} />
          </Card>
        </Col>
      </Row>
    </div>
