...
_RANDOM_TARGET_LOCK = threading.Lock()
_RANDOM_TARGETS: dict[tuple[str, tuple[int, ...]], list[list[int]]] = {}
...
def _parse_random_counts(raw):
    ...
    for index in range(NUM_POSITIONS):
        try:
            ...
            value = DEFAULT_RANDOM_COUNTS[index]
        counts.append(min(9, max(1, value)))
    return counts


def _random_targets_for_issue(issue, counts):
    key = (str(issue or "pending"), tuple(_parse_random_counts(counts)))
    with _RANDOM_TARGET_LOCK:
        targets = _RANDOM_TARGETS.get(key)
        if targets is None:
            picker = random.SystemRandom()
            targets = [sorted(picker.sample(range(10), count)) for count in key[1]]
            _RANDOM_TARGETS[key] = targets
            if len(_RANDOM_TARGETS) > 30:
                for old_key in list(_RANDOM_TARGETS)[:-20]:
                    _RANDOM_TARGETS.pop(old_key, None)
        return [list(numbers) for numbers in targets]
...
def _wait_for_next_cycle(page, account, stop_event, log):
    ...
    while not stop_event.is_set():
        ...
        if res and is_newer_issue(res[0], anchor_issue):
            log(f"[{account}] 已进入新周期 {res[0]}，开始执行随机码追损")
            return
        now = time.time()
        if now - last_beat >= 30:
...
def _betting_loop(page, account, cfg, stop_event, log):
    ...
    random_counts = _parse_random_counts(cfg.get("random_counts"))
    number_sets = DEFAULT_NUMBER_SETS
    ...
    sets_desc = "  ".join(
        ...
        else f"球{i+1}[每期随机{random_counts[i]}码]"
        for i in range(NUM_POSITIONS)
    )
    trigger_desc = "立即实投" if entry_misses == 0 else f"连续{entry_misses}次未中后实投"
    log(f"[{account}] 随机码追损启动 | 阶梯金额={amount_steps} 入场={trigger_desc} | {sets_desc} | 起始余额={start_balance}")
    if last_issue:
    ...
    while not stop_event.is_set():
        ...
        if res:
            ...
            if pending_settlement:
                ...
                _observe_entry_draw(paths, number_sets, draw, account, log, targets=_random_targets_for_issue(issue, random_counts), enabled_positions=enabled_positions)
                last_issue = issue
        ...
        if 0 <= cd <= win_max and not bet_placed:
            anchor = read_stable_draw(page, _get_last_draw_with_issue)
            if not anchor:
                log(f"[{account}] cannot read stable issue; skip current cycle")
                bet_placed = True
                remain = _get_countdown(page)
                _sleep_interruptible(max(30, remain) + draw_delay, stop_event)
                bet_placed = False
                continue
            targets = _random_targets_for_issue(anchor[0], random_counts)
            ...
            if not any(active_mask):
                observe_info = "  ".join(
                    f"球{i+1}随机{targets[i]}(观察{paths[i].entry_loss_count}/{paths[i].entry_miss_trigger})"
                    for i in range(NUM_POSITIONS)
            ...
            for i in range(NUM_POSITIONS):
                if not enabled_positions[i]:
                    ...
                    continue
                if active_mask[i]:
                    parts.append(f"球{i+1}随机{targets[i]}x{amounts[i]}元 第{paths[i].tier_index + 1}阶")
                else:
                    parts.append(f"球{i+1}随机{targets[i]}(观察{paths[i].entry_loss_count}/{paths[i].entry_miss_trigger})")
...
def _external_conflict(acc_info: dict):
    try:
        ...
        occupied = TaskManager.get().active_resources(exclude_task_id="random_rotatebet")
    except Exception:
        return None
...
def run(config: dict, stop_event: threading.Event, log_queue: queue.Queue):
    ...
    while not stop_event.is_set():
        latest = get_config("random_rotatebet_config", config)
        ...
        try:
            ...
            update_result = TaskManager.get().update_config("random_rotatebet", latest)
