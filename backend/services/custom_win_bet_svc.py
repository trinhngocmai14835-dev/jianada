"""Independent custom-amount rotate-win betting service."""
import asyncio
import queue
import re
import socket
...
class _CustomWinPathState:
    """State for one ball path in custom amount rotate-win mode."""
...
def _wait_for_next_cycle(page, account, stop_event, log):
    ...
    while not stop_event.is_set():
        ...
        if res and is_newer_issue(res[0], anchor_issue):
            log(f"[{account}] 已进入新周期 {res[0]}，开始执行自定义金额轮换赢冲")
            return
        now = time.time()
        if now - last_beat >= 30:
...
def _betting_loop(page, account, cfg, stop_event, log):
    ...
    log(f"[{account}] 自定义金额轮换赢冲启动 | 阶梯金额={amount_steps} 入场={trigger_desc} | {sets_desc} | 起始余额={start_balance}")
    if last_issue:
            ...
            if pending_settlement:
                ...
                if last_targets[0] is not None and is_newer_issue(issue, pending_issue):
                    ...
                    for i in range(NUM_POSITIONS):
                        ...
                        if hit:
                            reset = paths[i].on_win()
                            if reset:
                                log(f"[{account}]   球{i+1} 开{draw[i]}【中奖】 投{set_label}组{nums} | 本球注码={amount} -> 已完成最后一阶赢冲，回到第1阶，下一把注码={paths[i].get_bet()}")
                            else:
                                log(f"[{account}]   球{i+1} 开{draw[i]}【中奖】 投{set_label}组{nums} | 本球注码={amount} -> 进入第{paths[i].tier_index + 1}阶，下一把注码={paths[i].get_bet()}")
                        else:
                            paths[i].on_lose()
                            log(f"[{account}]   球{i+1} 开{draw[i]}【未中】 投{set_label}组{nums} | 本球注码={amount} -> 回到第1阶，下一把注码={paths[i].get_bet()}")

                    _observe_entry_draw(paths, number_sets, draw, account, log, targets=last_targets, enabled_positions=enabled_positions)
                    pending_settlement = False
                    pending_issue = None
                    last_issue = issue
...
def _external_conflict(acc_info: dict):
    try:
        ...
        occupied = TaskManager.get().active_resources(exclude_task_id="custom_winbet")
    except Exception:
        return None
...
def run(config: dict, stop_event: threading.Event, log_queue: queue.Queue):
    ...
    while not stop_event.is_set():
        latest = get_config("custom_winbet_config", config)
        ...
        try:
            ...
            update_result = TaskManager.get().update_config("custom_winbet", latest)
        ...
        for acc_info in accounts:
            ...
            if account and account in running_accounts:
                reason = f"账号 {account} 已在自定义赢冲模式中运行"
            elif port and port in running_ports:
                reason = f"端口 {port} 已在自定义赢冲模式中运行"
            else:
